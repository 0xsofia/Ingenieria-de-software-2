...
def listar_clases():
    socio_id = None
    if current_user.is_authenticated and getattr(current_user, "role", None) == "socio":
        socio_id = getattr(current_user, "persona_id", None)

    reservas_por_clase = {}
    if socio_id is not None:
        reservas = (
            Reserva.query.filter_by(socio_id=socio_id)
        ...
        for reserva in reservas:
            if reserva.clase_id not in reservas_por_clase:
                reservas_por_clase[reserva.clase_id] = reserva

    clases = (
        Clase.query.order_by(Clase.fecha_clase.asc(), Clase.hora_inicio.asc(), Clase.clase_id.asc())
        .all()
    )
        ...
        "items": [
            {
                ...
                "actividad": getattr(clase.actividad, "nombre", None),
                "fecha_clase": clase.fecha_clase.isoformat(),
                "hora_inicio": clase.hora_inicio.strftime("%H:%M"),
                "hora_fin": clase.hora_fin.strftime("%H:%M"),
                "cupo_total": clase.cupo_total,
                "cupo_disponible": _cupo_disponible(clase),
                "precio": float(clase.precio),
                "estado": clase.estado,
                "ya_reservado": clase.clase_id in reservas_por_clase,
                "reserva_id": getattr(reservas_por_clase.get(clase.clase_id), "reserva_id", None),
                "estado_reserva": getattr(reservas_por_clase.get(clase.clase_id), "estado", None),
            }
            for clase in clases
        ],
    }, 200
...
def iniciar_reserva_espontanea(clase_id):
    ...
    if (clase.estado or "activa").lower() != "activa":
        return {
            "status": "error",
    ...
    reserva = Reserva(
        ...
        estado="confirmada" if credito else "pendiente_pago",
        confirmada_en=_now() if credito else None,
    )
...
def _cupo_disponible(clase):
    ...
    return max(int(clase.cupo_total) - int(ocupados), 0)
...
def _crear_checkout_mercadopago(pago, reserva, clase):
    ...
    preference_payload = {
        "items": [
            {
                "title": f"Clase {getattr(clase.actividad, 'nombre', 'actividad')}",
                "quantity": 1,
                "currency_id": "ARS",
                "unit_price": float(clase.precio),
            }
        ],
