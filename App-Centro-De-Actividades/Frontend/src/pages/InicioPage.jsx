...
import { entrarListaEspera, listarClases, reservarEspontanea } from '../api/reservas'
import '../App.css'
...
function InicioPage() {
    ...
    async function load() {
      ...
      try {
        ...
        const firstDate = items
          .map((item) => parseIsoDate(item.fecha_clase))
          .filter(Boolean)
  ...
  async function handleReservar(claseOrEvent) {
    ...
    if (
      typeof claseOrEvent === 'object' &&
      claseOrEvent &&
      claseOrEvent.ya_reservado
    ) {
      setRequestError('')
                            ...
                            {calendarModel.events
                              ...
                              .map((event) => (
                                ...
                                >
                                  ...
                                  <span className="calendar-event__meta">
                                    Cupo: {event.cupo_disponible} / {event.cupo_total} • $ {event.precio}
                                  </span>
                                </button>
                              ))}
                          </div>
...
function buildCalendarModel(clases, weekStart) {
  ...
  const rawEvents = (Array.isArray(clases) ? clases : [])
    .map((item) => {
      const date = parseIsoDate(item.fecha_clase)
      if (!date) return null
